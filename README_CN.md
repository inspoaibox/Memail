...
Web 后台服务是 `mail-viewer`，Compose 默认只监听本机：

```text
http://127.0.0.1:5000
```

域名只需要反代到这个地址。SMTP 收信端口 `25` 和 IMAP 客户端端口 `993` 不是 HTTP 服务，不能用 Nginx/Caddy 的普通 `reverse_proxy` 代理；它们需要在防火墙和云服务器安全组里直接放行。

如果需要 Gmail / Outlook OAuth 回调或对外生成完整链接，请在 `.env` 中设置：
...
把 `mail.yourdomain.com` 换成你的实际域名：
...
设置页的“导入导出”可以生成 `.memail.json` 便携包。默认模式适合备份或分享配置，只包含账号档案、文件夹、邮件缓存、AI/翻译渠道信息和关键词规则，不包含外部邮箱密码、OAuth Token 或 API Key。

如果是迁移到自己的新服务器，勾选“完整迁移包：包含外部账号密码、OAuth Token、AI/翻译 API Key”。完整迁移包会额外导出：

- 外部 IMAP/SMTP 账号密码或应用专用密码。
- Gmail / Outlook OAuth refresh token。
- Gmail / Outlook OAuth Client Secret。
- AI 渠道 API Key、翻译服务密钥、Resend API Key、邮箱统一密码。

导入完整迁移包后，外部账号会写入新的 IMAP 桥接服务并在后台重新连接同步，不需要逐个重新填写密码或重新 OAuth 授权。完整迁移包包含敏感凭据，请只在迁移到自己服务器时使用，下载后妥善保存或及时删除。
